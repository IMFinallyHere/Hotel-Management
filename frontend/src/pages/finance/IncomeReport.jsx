...
import { REPORT_QUERY_KEYS, fetchIncomeReport } from '../../api/queries';
...
export default function IncomeReport() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: REPORT_QUERY_KEYS.incomeReport(params),
  ...
  return (
    <Stack gap="md">
      <Group justify="space-between" align="flex-end">
        <Text fw={700} size="xl">Income Report</Text>
        <DatePickerInput
          type="range"
          label="Date range"
          value={dates}
        ...
        <>
          <SimpleGrid cols={Math.max(1, Object.keys(data.by_payment_type).length + 1)} spacing="md">
            <Card withBorder p="md">
              ...
              <Text fw={700} size="xl" c="teal">₹{data.total.toLocaleString()}</Text>
            </Card>
          ...
          {data.by_day.length === 0 && (
            <Text c="dimmed" ta="center">No income in this date range.</Text>
          )}
        </>
      )}
    </Stack>
