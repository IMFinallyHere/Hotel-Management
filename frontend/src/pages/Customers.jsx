...
export default function Customers() {
  ...
  const rows = data.map((customer) => (
    <Table.Tr key={customer.id}>
      ...
      <Table.Td>
        <Group gap="xs">
          <Button size="xs" variant="light" onClick={() => openEdit(customer)}>Edit</Button>
          <Button size="xs" color="red" variant="light" onClick={() => handleDelete(customer.id)}>Delete</Button>
        </Group>
      </Table.Td>
    </Table.Tr>
  ));
  ...
  return (
    <>
      ...
      <Modal opened={opened} onClose={close} title={editing ? 'Edit Customer' : 'Add Customer'} size="lg">
        <form onSubmit={form.onSubmit(v => saveMutation.mutate(v))}>
          <Stack gap="sm">
            ...
            <Select
              ...
              required
            />
            <TextInput label="Phone Number" maxLength={10} {...form.getInputProps('number')} required />
            ...
            <TextInput label="Pincode" maxLength={6} {...form.getInputProps('pincode')} />
            <FileInput
