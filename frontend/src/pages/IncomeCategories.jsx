...
import { QUERY_KEYS, fetchIncomeCategories } from '../api/queries';
...
export default function IncomeCategories() {
  ...
  const canAdd    = permissions.add_incomecategory    || permissions.is_superuser;
  const canChange = permissions.change_incomecategory || permissions.is_superuser;
  const canDelete = permissions.delete_incomecategory || permissions.is_superuser;

  const { data = [], isLoading } = useQuery({ queryKey: QUERY_KEYS.incomeCategories, queryFn: () => fetchIncomeCategories() });
  ...
  const saveMutation = useMutation({
    mutationFn: (values) => editing
      ? api.put(`/v1/income-categories/${editing.id}/`, values)
      : api.post('/v1/income-categories/', values),
    onSuccess: () => { qc.invalidateQueries({ queryKey: QUERY_KEYS.incomeCategories }); close(); notifySuccess('Saved.'); },
    onError: (e) => notifyError(parseApiError(e, 'Failed to save.')),
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => api.delete(`/v1/income-categories/${id}/`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: QUERY_KEYS.incomeCategories }); notifySuccess('Deleted.'); },
        ...
        <Table striped highlightOnHover withTableBorder>
          ...
          <Table.Tbody>
            {isLoading ? (
              ...
              <Table.Tr><Table.Td colSpan={3} ta="center">No income categories yet.</Table.Td></Table.Tr>
            ) : rows}
          </Table.Tbody>
        </Table>
