...
import { QUERY_KEYS, QUERY_KEYS_OPS, fetchIncome, fetchPaymentMethods, fetchIncomeCategories } from '../api/queries';
...
export default function Income() {
  ...
  const canAdd = permissions.add_income || permissions.is_superuser;
  const canChange = permissions.change_income || permissions.is_superuser;
  const canDelete = permissions.delete_income || permissions.is_superuser;
  const canCreateCategory = permissions.add_incomecategory || permissions.is_superuser;
  ...
  const { data: income = [], isLoading } = useQuery({
    queryKey: QUERY_KEYS_OPS.income(queryParams),
    queryFn: () => fetchIncome(queryParams),
  });
  ...
  const { data: categories = [] } = useQuery({ queryKey: QUERY_KEYS.incomeCategories, queryFn: () => fetchIncomeCategories() });
  ...
  const createCategoryMutation = useMutation({
    mutationFn: (name) => api.post('/v1/income-categories/', { name, is_active: true }),
    onSuccess: (res) => {
      qc.invalidateQueries({ queryKey: QUERY_KEYS.incomeCategories });
  ...
  const uploadAttachments = async (incomeId, files) => {
    ...
    await api.post(`/v1/income/${incomeId}/attachments/`, fd, {
  ...
  const saveMutation = useMutation({
    mutationFn: async (values) => {
      ...
      const res = editing
        ? await api.put(`/v1/income/${editing.id}/`, payload)
        : await api.post('/v1/income/', payload);
      return res.data;
    },
    onSuccess: async (data) => {
      ...
      try {
        ...
        notifyError('Income saved but some attachments failed — try re-uploading.');
      } finally {
  ...
  const totalForDay = income.reduce((s, i) => s + Number(i.amount), 0);

  const rows = income.map((inc) => (
    <Table.Tr key={inc.id}>
      <Table.Td>{inc.date}</Table.Td>
      <Table.Td>
        <Badge variant="light" color="teal">
          {catMap[inc.category] ?? inc.category_name ?? '—'}
        </Badge>
      </Table.Td>
      <Table.Td>{inc.description || <Text size="xs" c="dimmed">—</Text>}</Table.Td>
      <Table.Td>₹{inc.amount}</Table.Td>
      <Table.Td>
        <Badge variant="light" color="blue">
          {pmMap[inc.payment_method] ?? inc.payment_method_name ?? '—'}
        </Badge>
      </Table.Td>
      <Table.Td>{inc.recorded_by_name ?? '—'}</Table.Td>
      <Table.Td>
      ...
      <Table.Td>
        <Group gap="xs">
          {canChange && <Button size="xs" variant="light" onClick={() => openEdit(inc)}>Edit</Button>}
          {canDelete && <Button size="xs" color="red" variant="light" onClick={() => handleDelete(inc.id)}>Delete</Button>}
        </Group>
      </Table.Td>
    </Table.Tr>
  ));
        ...
        <Table striped highlightOnHover withTableBorder>
          ...
          <Table.Tbody>
            {isLoading ? (
              ...
              <Table.Tr><Table.Td colSpan={8} ta="center">No income recorded for this date.</Table.Td></Table.Tr>
            ) : rows}
          </Table.Tbody>
        </Table>
      </Table.ScrollContainer>

      <Modal opened={opened} onClose={close} title={editing ? 'Edit Income' : 'Add Income'} size={{ base: '95%', sm: 'lg' }}>
